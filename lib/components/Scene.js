module.exports.register = function (cml, modules) {
    ...
    var ogRefresh = cmlruntime.refresh;
    var ogInit = cmlruntime.init;

    cmlruntime.sceneGraph = function () {
        var graph = {};
        var makePath = function (scene) {
            return (cml.extract(scene, "basePath") || "") + cml.extract(scene, "path");
        };
        var makeNodes = function (scene) {

        }
        for (var k in _scenes) {
            var scene = _scenes[k];
            var edges = cml.extract(scene, "edges") || [];
            graph[makePath(scene)] = edges.map(function (e) {
                return makePath(scene);
            });
        }
        return graph;
    };
