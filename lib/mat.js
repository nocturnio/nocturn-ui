...
var replaceClass = function (el, obj, name, c2, preText, postText, haveMore) {
    ...
    var c1 = obj[name];
    if (c1) {
        c1.toString().split(" ").forEach(function (c) {
            elem.removeClass(el, preText + c + postText);
        });
    }
    if (c2) {
        c2.toString().split(" ").forEach(function (c) {
            elem.appendClass(el, preText + c + postText);
        });
    }
    if (!haveMore) {
        obj[name] = c2;
...
var fabA = function (el) {
    return elem.byClassName(el, "fab-btn-main")[0];
};
...
var fileInputField = function (el) {
    return elem.byTagName(el, "input").filter(function (el) {
        return el.type == "file";
    })[0];
};
...
var navTitle = function (el) {
    return elem.byClassName(el, "brand-logo")[0];
};
var sideNavTitle = function (el) {
    return elem.byTagName(elem.byClassName(el, "side-nav-title")[0], "a")[0];
};
...
var getMdRenderer = function () {
    if (mdRenderer) return mdRenderer;

    mdRenderer = new Remarkable({
        html: false,
        xhtmlOut: false,
        breaks: false,
        langPrefix: "language-",
        ...
        highlight: function (str, lang) {
            var rtn = "";
            if (lang && lang === "text") {
                var encodedStr = elem.m("div", {}, str).innerHTML;
                rtn = encodedStr;
...
var markdown = function (options) {
    var op = options;
    var html = getMdRenderer().render(op.value);
    var cc = colorClassOptions(op);
    var el = elem.m("div", {
        class: "markdown " + cc
    });
...
var code = function (options) {
    var op = options;
    var html = getMdRenderer().render(codeMd(op.type, op.value));
    var cc = colorClassOptions(op);
    var el = elem.m("div", {
        class: "markdown " + cc
    });
...
var fabItem = function (options) {
    ...
    var cc = colorClassOptions(op);
    var aEl = elem.m("a", {
        class: "btn-floating " + cc
    }, elem.m("i", {
        class: "material-icons"
    }, op.icon));

    if (op.disabled) {
        elem.appendClass(aEl, "disabled");
    }

    if (op.click) {
        aEl.onclick = function (e) {
            if (!elem.haveClass(aEl, "disabled")) {
                op.click(e);
            }
        };
    }
    ...
    /*
    <div id="fixed-action-button" class="fixed-action-btn horizontal click-to-toggle toolbar">
...
var sideNavItem = function (options) {
    ...
    elem.appendClass(container, "side-nav-item");
    if (op.key) {
        elem.appendClass(container, "__sidenavitem-" + op.key + "__");
    }
    if (op.secondary) {
    ...
    if (op.select) {
        container.onclick = function (e) {
            op.select(e);
            elem.byClassName(elem.byId("side-nav"), "side-nav-item")
                .forEach(function (c) {
...
var linkItem = function (options) {
    ...
    elem.appendClass(linkEl, "clickable");
    return linkEl;
};
...
var navbar = function (options) {
    ...
    if (is.undefined(container)) {
        container = elem.m("div", { id: "navbar" });
        elem.append(elem.byId("header"), container);
    }
    ...
    if (dom.items && dom.items.length > 0) {
        var itemsEl = elem.m("ul", { class: "right hide-on-med-and-down navlinks" });
        var dropdownId = op.dropdownId;
        var dropdownEl = navDropdown({
            id: dropdownId
        });
        dom.items.forEach(function (c) {
            elem.append(itemsEl, elem.m("li", {}, c));
            elem.append(dropdownEl, elem.m("li", {}, c.cloneNode(true)));
        });
        var dropdownButtonEl = navDropdownButton({
            icon: "more_vert",
            id: dropdownId
        });
        elem.append(wrapEl, itemsEl);
        elem.appendClass(dropdownButtonEl, "hide-on-large-only navlinks-dropdown-button");
        elem.append(wrapEl, elem.m("ul", { class: "right navlinks-dropdown" }, elem.m("li", {}, dropdownButtonEl)));
        dropdownLoad({
...
var sidenav = function (options) {
    ...
    if (op.sections) {
        ...
        if (op.expandable) {
            accordianEl.setAttribute("data-collapsible", "expandable");
        }
        elem.append(sideNavEl, elem.m("li", {
            class: "no-padding"
        }, accordianEl));
        objForEachKey(op.sections, function (k) {
            var section = op.sections[k];
            var label = section.label;
            var collapsedBody = elem.m("div", {
                class: "collapsible-body"
            });
            var headerEl = elem.m("a", {
                class: "collapsible-header side-nav-section"
            }, label);
            if (section.icon) {
            ...
            if (section.secondary) {
                section.secondary.reverse().forEach(function (sec) {
                    elem.append(headerEl, listItemSecondary(sec));
                });
            }
...
var modal = function (options) {
    ...
    makeActions(op, container, "modal-footer " + cc + op.moduleName + "-footer");
    makeCollapsibles(op);
    if (op.click) {
        container.onclick = function (e) {
            op.click(e);
...
var fab = function (options) {
    ...
    var aEl = elem.m("a", {
        class: "btn-floating btn-large waves-effect waves-light fab-btn-main " + cc
    }, elem.m("i", {
    ...
    if (op.clickToggle) {
        elem.appendClass(el, "click-to-toggle");
    }
    if (op.horizontal) {
        elem.appendClass(el, "horizontal");
    }
    if (op.toolbar) {
        elem.appendClass(el, "toolbar");
    }
    ...
    if (op.container) {
        var containerEl = getContainer(op);
        elem.append(containerEl, el);
    } else {
        // global FAB
        elem.append(elem.byId("main"), el);
    }
...
var card = function (options) {
    ...
    if (op.click) {
        ...
        elem.appendClass(cardEl, "clickable");
    }
    return el;
};
var getContainer = function (options) {
    ...
    if (!container) {
        if (containerId == "__page1__") {
            container = elem.m("div", {
                id: containerId,
                class: "page animated row"
            });
        } else {
            container = elem.m("div", {
                 id: containerId,
                 class: "page animated display-none row"
             });
        }
...
var sideNavSectionLoad = function (options) {
    ...
    if (op.select) {
        ...
        var headerEl = elem.byClassName(el, "side-nav-section")[0];
        headerEl.onclick = function (e) {
            op.select(e);
        };
    }
    if (op.active) {
        sideNavSectionActivate(options)
    }
};
...
var markdownRender = function (el, md) {
    el.innerHTML = getMdRenderer().render(md);
...
_animationState = {
    ...
    _count: 0,
    isAnimating: false,
    queueAnimation: function (el, animation, timer, delay) {
        var state = {
            isCanceled: false,
            isDone: false
        };
        ...
        this._count++;
        animateEl(el, animation, timer, this._delay, state);
    },
    cancelCurrent: function () {
        this._queue.forEach(function (state) {
...
var animateEl = function(el, animation, timer, delay, state) {
    timer = timer || 500;
    delay = delay || 0;
    elem.appendClass(el, "animated");
    elem.appendClass(el, animation);
    setTimeout(function () {
        if (!state.isCanceled) {
            elem.removeClass(el, "display-none");
...
module.exports = {
    ...
    sideNavUnshiftBody: sideNavUnshiftBody,
    sideNavShiftBody: sideNavShiftBody,
